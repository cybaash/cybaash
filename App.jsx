...
const CRED_CSS = `
  ...
  .bulk-modal-overlay{position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:2000;display:flex;align-items:center;justify-content:center;padding:20px}
  .bulk-modal{background:var(--bg1);border:1px solid var(--bd);border-radius:4px;width:100%;max-width:700px;max-height:90vh;overflow-y:auto;padding:28px}
  .bulk-title{font-family:'Share Tech Mono',monospace;font-size:14px;color:var(--g);letter-spacing:3px;margin-bottom:20px;padding-bottom:12px;border-bottom:1px solid var(--bd)}
  .bulk-section{margin-bottom:24px}
  .bulk-section-label{font-family:'Share Tech Mono',monospace;font-size:11px;letter-spacing:2px;color:var(--tx2);margin-bottom:10px}
  .bulk-issuer-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}
  .bulk-issuer-row{display:flex;align-items:center;gap:8px;padding:6px 10px;background:var(--bg2);border:1px solid var(--bd);border-radius:3px}
  .bulk-issuer-name{font-size:11px;color:var(--tx2);flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
  .bulk-issuer-status{font-size:10px;width:16px;text-align:center}
  .bulk-file-input{display:none}
  .bulk-file-btn{font-size:10px;padding:3px 8px;background:transparent;border:1px solid var(--bd);color:var(--tx3);cursor:pointer;font-family:monospace;border-radius:2px;white-space:nowrap}
  .bulk-file-btn:hover{border-color:var(--g);color:var(--g)}
  .bulk-file-btn.loaded{border-color:var(--g);color:var(--g)}
  .bulk-log{background:var(--bg);border:1px solid var(--bd);border-radius:3px;padding:12px;max-height:180px;overflow-y:auto;font-size:11px;line-height:1.8;font-family:'Share Tech Mono',monospace;margin-top:12px}
  .bulk-log-ok{color:var(--g)}.bulk-log-err{color:var(--red)}.bulk-log-info{color:var(--amber)}
  .bulk-bar-wrap{background:var(--bg);border:1px solid var(--bd);height:6px;border-radius:3px;margin-top:10px;overflow:hidden}
  .bulk-bar{height:100%;background:var(--g);border-radius:3px;width:0%;transition:width .3s}
`

// ── KNOWN ISSUERS for logo upload ─────────────────────────────────────────
const KNOWN_ISSUERS = [
  'Udemy','Cybrary','Zendesk','Wolfram','GitHub','ServiceNow',
  'Cisco Networking Academy','Adobe','LinkedIn','Grammarly','Aha!',
  'American Negotiation Institute','Jack Welch Management Institute',
  'Toastmasters International','All Tech Is Human','LambdaTest',
  'Weaviate','Docker, Inc','KNIME','Snowflake','Anaconda, Inc.',
  'Canonical','Microsoft','OpenEDG Python Institute','Mozilla',
  'OpenEDG C++ Institute','JetBrains','Amazon Web Services',
]

function BulkImportModal({ creds, onSave, onClose }) {
  const [log, setLog]       = useState([])
  const [running, setRunning] = useState(false)
  const [progress, setProgress] = useState(0)
  const [logoMap, setLogoMap]   = useState({}) // issuer -> base64
  const csvRef  = useRef()

  const addLog = (type, msg) => setLog(p => [...p, { type, msg }])

  // ── CSV: update cert URLs ──────────────────────────────────────────────
  const handleCSV = e => {
    ...
    reader.onload = async ev => {
      ...
      if (nameIdx < 0 || urlIdx < 0) { addLog('err','CSV must have Name and Url columns'); return }

      setRunning(true)
      addLog('info', `Parsing ${lines.length - 1} rows from CSV…`)
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const idx = newCreds.findIndex(c => c.title?.trim().toLowerCase() === name.toLowerCase())
        if (idx >= 0) { newCreds[idx] = { ...newCreds[idx], url }; updated++ }
        else notFound++
        setProgress(Math.round((i / (lines.length - 1)) * 50))
      }

      addLog('ok', `URLs matched: ${updated} updated, ${notFound} not found`)
      addLog('info', 'Saving to Supabase…')
      await onSave(newCreds)
      setProgress(100)
      addLog('ok', '✓ All cert URLs saved!')
      setRunning(false)
    }
    reader.readAsText(file)
  }

  // ── Logo: single image file per issuer ────────────────────────────────
  const handleLogoFile = (issuer, e) => {
    const file = e.target.files[0]
    if (!file) return
    const reader = new FileReader()
    reader.onload = ev => {
      setLogoMap(p => ({ ...p, [issuer]: ev.target.result }))
    }
    reader.readAsDataURL(file)
  }

  const applyLogos = async () => {
    const issuers = Object.keys(logoMap)
    if (!issuers.length) { addLog('err', 'No logo images loaded'); return }
    setRunning(true)
    addLog('info', `Applying logos for ${issuers.length} issuer(s)…`)
    let newCreds = [...creds]
    let total = 0
    issuers.forEach(issuer => {
      const b64 = logoMap[issuer]
      newCreds = newCreds.map(c => {
        if (c.issuer?.trim().toLowerCase() === issuer.trim().toLowerCase()) {
          total++
          return { ...c, logo: b64 }
        }
        return c
    ...
    addLog('ok', `Logos applied to ${total} cert(s). Saving…`)
    await onSave(newCreds)
    addLog('ok', '✓ Logos saved!')
    setRunning(false)
  }
    ...
    <div className="bulk-modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bulk-modal">
        ...
        <div className="bulk-section">
          <div className="bulk-section-label">STEP 1 — UPDATE CERT URLs FROM CSV</div>
          <div style={{ display:'flex', gap:10, alignItems:'center' }}>
            <input ref={csvRef} type="file" accept=".csv" className="bulk-file-input" onChange={handleCSV} />
            <button className="btn" style={{ fontSize:12 }} disabled={running} onClick={() => csvRef.current.click()}>
              📂 Upload Certifications.csv
            </button>
            <span style={{ fontSize:11, color:'var(--tx3)' }}>Columns needed: Name, Url</span>
          </div>
        </div>

        {/* ── Logos ── */}
        <div className="bulk-section">
          <div className="bulk-section-label">STEP 2 — UPDATE LOGOS BY ISSUER</div>
          <div className="bulk-issuer-grid">
            {KNOWN_ISSUERS.map(issuer => (
              <div key={issuer} className="bulk-issuer-row">
                <span className="bulk-issuer-status">{logoMap[issuer] ? '✓' : '○'}</span>
                <span className="bulk-issuer-name" title={issuer}>{issuer}</span>
                <label className={`bulk-file-btn${logoMap[issuer] ? ' loaded' : ''}`}>
                  {logoMap[issuer] ? 'Change' : 'Choose'}
                  <input type="file" accept="image/*" style={{ display:'none' }}
                    onChange={e => handleLogoFile(issuer, e)} />
                </label>
              </div>
            ))}
          </div>
          <button
            className="btn"
            style={{ marginTop:12, width:'100%', borderColor:'var(--g)', color:'var(--g)' }}
            disabled={running || !Object.keys(logoMap).length}
            onClick={applyLogos}
          >
            ▶ Apply {Object.keys(logoMap).length} Logo(s) to All Matching Certs
          </button>
        </div>

        {/* ── Progress + Log ── */}
        {progress > 0 && (
          <div className="bulk-bar-wrap"><div className="bulk-bar" style={{ width: progress+'%' }} /></div>
        )}
        {log.length > 0 && (
          <div className="bulk-log">
            {log.map((l, i) => (
              <div key={i} className={`bulk-log-${l.type}`}>
  ...
  return (
    <div>
      ...
      {bulkOpen && (
        <BulkImportModal
          creds={creds}
          onSave={async updated => { await commit(updated) }}
